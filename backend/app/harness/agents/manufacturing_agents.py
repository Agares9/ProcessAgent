...
class ManufacturingIntentAgent:
    ...
    def infer(self, query: str, context: str = "") -> ManufacturingIntent:
        ...
        if any(k in text for k in ("你能做什么", "你会什么", "你的功能", "你是谁", "你是什么助手")):
            return ManufacturingIntent(intent_type="capability_query", domain="capability", response_mode="capability_info", complexity="simple", confidence=0.99, raw={"source": "deterministic_fallback", "query": query})
        if text in {"你好", "您好", "嗨", "hello", "hi"}:
            return ManufacturingIntent(intent_type="chitchat", domain="general_chat", response_mode="boundary_redirect", complexity="simple", confidence=0.99, raw={"source": "deterministic_fallback", "query": query})
        objectives: list[str] = []
        mappings = {
        ...
        return ManufacturingIntent(
            ...
            domain="manufacturing", response_mode="analysis",
    ...
    async def infer_async(self, query: str, llm: LLMClient | None = None, context: str = "") -> ManufacturingIntent:
        ...
        prompt = ("你是场景端意图识别助手，只输出JSON。字段：domain(manufacturing|capability|general_chat|out_of_scope), response_mode(analysis|capability_info|boundary_redirect), intent_type, complexity(simple|standard|complex), objectives, industries, "
...
class LeadAgent:
    ...
    async def plan_async(self, intent: ManufacturingIntent, context: EnterpriseContext, llm: LLMClient | None = None) -> AnalysisPlan:
        ...
        try:
            ...
            allowed = {"search_manufacturing_knowledge", "search_case_studies", "get_document_evidence", "check_applicability", "compare_technical_options", "extract_process_parameters", "calculate_project_financials", "calculate_energy_savings", "calculate_emission_reduction", "verify_citations", "check_constraint_compliance"}
...
class OrchestratorAgent:
    ...
    def synthesize(self, query: str, results: list[Any], verification: dict[str, Any]) -> dict[str, Any]:
        ...
        return {
            "executive_summary": "未通过证据校验，暂不输出确定性结论。" if not verification.get("passed") else "已基于检索到的制造业资料形成初步分析，具体措施仍需结合企业现场数据验证。",
    ...
    async def synthesize_async(self, query: str, results: list[Any], verification: dict[str, Any], llm: LLMClient | None = None) -> dict[str, Any]:
        ...
        prompt = ("你是制造业证据增强回答Agent。请直接用自然、清晰的中文回答用户，不要输出JSON，不要套用固定报告模板。"
        ...
        try:
            content = await llm.complete([ChatMessage.system("你是流畅、严谨的制造业回答助手。只输出最终回答，不输出JSON和推理过程。"), ChatMessage.user(prompt)], temperature=0.3)
